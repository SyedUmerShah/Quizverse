...
function quiz(topic) {
    ...
    for (var i = 0; i < answer.length; i++) {
        if (answer[i].checked) {
            ...
            var userAnswer = whichTopic[index - 1][`option${selected}`]
            var correctAnswer = whichTopic[index - 1].correctOption
    ...
    if (index >= 20) {
        ...
        if (result < 70) {
            var condition = "Sorry! You Are Fail"
        }
        else {
            var condition = "Congratulations! You Are Pass"

        }
        quiz.innerHTML = `<div class="upper">${condition}<div class="result">${result}%</div></div>`

    } else {
        ques.innerHTML = whichTopic[index].question
        option1.innerText = whichTopic[index].option1
        option2.innerText = whichTopic[index].option2
        option3.innerText = whichTopic[index].option3
        index++
        min = 1
        sec = 59
    }
}
function enableBtnNext(params) {
    btn.disabled = false
}
